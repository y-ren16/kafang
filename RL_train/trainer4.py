...
class MarketmakingTrainer(BasicActirCriticTrainer):
    ...
    def extract_state(self, all_observes) -> np.ndarray:
        ...
        # import pdb
        # pdb.set_trace()
        mid_price = (state['bp0'] + state['ap0']) / 2
        f0 = ((state['ap0'] - mid_price) * state['av0']) / ((mid_price - state['bp0']) * state['bv0'])
        f1 = ((state['ap1'] - mid_price) * state['av1']) / ((mid_price - state['bp1']) * state['bv1'])
        f2 = ((state['ap2'] - mid_price) * state['av2']) / ((mid_price - state['bp2']) * state['bv2'])
        f3 = ((state['ap3'] - mid_price) * state['av3']) / ((mid_price - state['bp3']) * state['bv3'])
        f4 = ((state['ap4'] - mid_price) * state['av4']) / ((mid_price - state['bp4']) * state['bv4'])

        state = np.array([f0, f1, f2, f3, f4, state['signal0'], state['signal1'], state['signal2']])
        ...
        #                   state['ap0'], state['ap1'], state['ap2'], state['ap3'], state['ap4']])
        # state[3:8] = state[3:8] / ap0_t0
        # state[8:13] = state[8:13] / ap0_t0
        return state
