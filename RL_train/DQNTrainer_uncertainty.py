...
class EnsembleLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        bias = self.bias
        if len(x.shape) == 2:
            x = torch.einsum('ij,bjk->bik', x, weight)  # b表示num_ensemble, j表示输入的特征维度，k表示输出的特征维度，i表示batch_size
        elif len(x.shape) == 3:
...
class ensembleQNetwork(nn.Module):
    def __init__(self,
        ...
        mlp_hidden_size = [state_dim] + list(mlp_hidden_size)
        ...
        self.predictor.append(EnsembleLinear(mlp_hidden_size[-1], action_dim, num_ensemble,
                                             weight_decays[-1]))

    def forward(self, state):
        pre = state
        if len(pre.shape) == 1:
            pre = pre.unsqueeze(0).expand(self.num_ensemble, -1)  # 增加维度使得shape=(batch_size, feature_size)
        for i in range(len(self.predictor)):
            pre = self.predictor[i](pre)
...
class DQNTrainer_uncertainty:
    def __init__(self, state_dim, critic_mlp_hidden_size, critic_lr, gamma, soft_tau,
        ...
        self.critic = ensembleQNetwork(state_dim=state_dim,
                                       ...
                                       mlp_hidden_size=critic_mlp_hidden_size,
                                       num_ensemble=num_ensemble).to(device)
    ...
    def get_action(self, state):
        q = self.critic(state)
        q = torch.mean(q, dim=0) - self.beta * torch.var(q, dim=0)
    ...
    def critic_train_step(self, batch_size):
        ...
        with torch.no_grad():
            ...
            next_q = torch.mean(self.target_critic(next_state), dim=0)[torch.arange(batch_size), next_action]
    ...
    def RL_train(self, save_dir, rl_step, batch_size, init_noise=0.5, noise_dumping=0.99):
        ...
        for i in range(int(rl_step) + 1):
            ...
            if random.random() < noise:
                ...
                action = self.get_action(state=torch.FloatTensor(state).to(self.device))[0].item()
    ...
    def RL_test(self, test_length=100000):
        ...
        for _ in range(test_length):
            ...
            state = torch.FloatTensor(state).to(self.device)
            action = self.get_action(state)[0].item()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--critic-mlp-hidden-size", type=tuple, help="number of hidden units per layer in critic",
                        default=(512, 512))
    ...
    parser.add_argument("--beta", type=float, help='在选择动作时对Q函数的方差进行惩罚，来选择方差更小、更有把握的动作。这一参数可能需要较多调试', default=0.01)
    ...
    trainer = DQNTrainer_uncertainty(state_dim=state_dim,
